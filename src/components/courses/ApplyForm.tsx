...
const ApplyForm: React.FC = () => {
  ...
  try {
    // 👉 STEP 1: Send form data to backend (before payment)
    const applyRes = await fetch('https://kgpath.onrender.com/api/apply', {
      method: 'POST',
    ...
    const orderRes = await fetch('https://kgpath.onrender.com/api/create-order', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
    ...
    setTimeout(() => handlePayment(data, order), 1000);
  ...
  const options: RazorpayOptions = {
    ...
    handler: async (response: RazorpayResponse) => {
      ...
      // 👉 Send payment verification + form data
      await fetch('https://kgpath.onrender.com/api/verify-payment', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
